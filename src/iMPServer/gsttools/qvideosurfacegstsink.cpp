...
Q_GLOBAL_STATIC_WITH_ARGS(QMediaPluginLoader, bufferPoolLoader,
        (QGstBufferPoolInterface_iid, iLatin1String("video/bufferpool"), Qt::CaseInsensitive))
    ...
    , m_startCanceled(false)
{
    if (m_surface) {
        ...
        for (iObject *instance : instances) {
            iGstBufferPoolInterface* plugin = qobject_cast<iGstBufferPoolInterface*>(instance);
...
std::list<iVideoFrame::PixelFormat> QVideoSurfaceGstDelegate::supportedPixelFormats(iAbstractVideoBuffer::HandleType handleType) const
{
    iScopedLock locker(const_cast<iMutex *>(&m_mutex));

    if (!m_surface)
        return std::list<iVideoFrame::PixelFormat>();
    else if (handleType == iAbstractVideoBuffer::NoHandle)
...
iVideoSurfaceFormat QVideoSurfaceGstDelegate::surfaceFormat() const
{
    iScopedLock locker(const_cast<iMutex *>(&m_mutex));
    return m_format;
}

bool QVideoSurfaceGstDelegate::start(const iVideoSurfaceFormat &format, int bytesPerLine)
{
    if (!m_surface)
        return false;

    iScopedLock locker(&m_mutex);
...
{
    iScopedLock locker(&m_poolMutex);
    if (m_pool)
        m_pool->clear();
}
...
GstFlowReturn QVideoSurfaceGstDelegate::render(GstBuffer *buffer)
{
    ...
    iAbstractVideoBuffer *videoBuffer = 0;

    if (m_pool)
        videoBuffer = m_pool->prepareVideoBuffer(buffer, m_bytesPerLine);

    if (!videoBuffer)
        videoBuffer = new iGstVideoBuffer(buffer, m_bytesPerLine);
    ...
    iGstUtils::setFrameTimeStamps(&m_frame, buffer);
...
{
    iGstBufferPoolInterface *newPool = 0;
    for (iGstBufferPoolInterface *pool : qAsConst(m_pools)) {
...
GType QVideoSurfaceGstSink::get_type()
{
    ...
    if (type == 0) {
        static const GTypeInfo info =
        {
            ...
            base_init,                                         // base_init
            IX_NULLPTR,                                              // base_finalize
            class_init,                                        // class_init
            IX_NULLPTR,                                              // class_finalize
            IX_NULLPTR,                                              // class_data
...
void QVideoSurfaceGstSink::instance_init(GTypeInstance *instance, gpointer g_class)
{
    ...
    IX_GCC_UNUSED(g_class);

    sink->delegate = 0;

    sink->lastRequestedCaps = 0;
    sink->lastBufferCaps = 0;
    sink->lastSurfaceFormat = new iVideoSurfaceFormat;
}
...
void QVideoSurfaceGstSink::handleShowPrerollChange(GObject *o, GParamSpec *p, gpointer d)
{
    ...
    g_object_get(G_OBJECT(sink), "show-preroll-frame", &showPrerollFrame, IX_NULLPTR);

    if (!showPrerollFrame) {
        GstState state = GST_STATE_VOID_PENDING;
        gst_element_get_state(GST_ELEMENT(sink), &state, IX_NULLPTR, GST_CLOCK_TIME_NONE);
...
GstStateChangeReturn QVideoSurfaceGstSink::change_state(GstElement *element, GstStateChange transition)
{
    ...
    gboolean showPrerollFrame = true; // "show-preroll-frame" property is true by default
    g_object_get(G_OBJECT(element), "show-preroll-frame", &showPrerollFrame, IX_NULLPTR);
...
GstCaps *QVideoSurfaceGstSink::get_caps(GstBaseSink *base)
{
    ...
    std::list<iVideoFrame::PixelFormat> supportedFormats;

    std::list<iVideoFrame::PixelFormat> poolHandleFormats;
    sink->delegate->poolMutex()->lock();
    iGstBufferPoolInterface *pool = sink->delegate->pool();
    ...
    for (iVideoFrame::PixelFormat format : supportedPixelFormats) {
        if (!poolHandleFormats.contains(format))
            supportedFormats.append(format);
    }

    return iGstUtils::capsForFormats(supportedFormats);
}
...
gboolean QVideoSurfaceGstSink::set_caps(GstBaseSink *base, GstCaps *caps)
{
    ...
    if (!caps) {
        ...
        iAbstractVideoBuffer::HandleType handleType =
                pool ? pool->handleType() : iAbstractVideoBuffer::NoHandle;

        iVideoSurfaceFormat format = iGstUtils::formatForCaps(caps, &bytesPerLine, handleType);

        if (sink->delegate->isActive()) {
            iVideoSurfaceFormat surfaceFormst = sink->delegate->surfaceFormat();
        ...
        GstBaseSink *base, guint64 offset, guint size, GstCaps *caps, GstBuffer **buffer)
{
    ...
    *buffer = IX_NULLPTR;
    ...
    iScopedLock poolLock(sink->delegate->poolMutex());
    iGstBufferPoolInterface *pool = sink->delegate->pool();
    ...
    if (sink->delegate->isActive()) {
        ...
        iVideoSurfaceFormat format = iGstUtils::formatForCaps(intersection);
        iVideoSurfaceFormat surfaceFormat = sink->delegate->surfaceFormat();
    ...
    if (!sink->delegate->isActive()) {
        ...
        iVideoSurfaceFormat format = iGstUtils::formatForCaps(intersection, &bytesPerLine, handleType);

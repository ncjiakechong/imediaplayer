                          ...
                          GstVideoConnectorClass *g_class)
{
    ...
    element->latest_buffer = IX_NULLPTR;
}
...
gst_video_connector_reset (GstVideoConnector * element)
{
    element->relinked = FALSE;
    element->failedSignalEmited = FALSE;
    if (element->latest_buffer != IX_NULLPTR) {
                                  ...
                                  GstCaps * caps, GstBuffer ** buf)
{
    ...
    *buf = IX_NULLPTR;
    ...
    while (1) {
        ...
        // Check if downstream element is in IX_NULLPTR state
        ...
        if (peerPad) {
            ...
            if (parent) {
                ...
                if (state == GST_STATE_NULL) {
                    GST_DEBUG_OBJECT (element, "Downstream element is in IX_NULLPTR state");

...
public:
    ...
    iSize sizeHint() const override
    {
        return m_nativeSize;
    }

    void setNativeSize( const iSize &size)
    {
...
QGstreamerVideoWidgetControl::QGstreamerVideoWidgetControl(iObject *parent, const iByteArray &elementName)
...
void QGstreamerVideoWidgetControl::stopRenderer()
{
    m_stopped = true;
    updateWidgetAttributes();
    m_widget->setNativeSize(iSize());
}

void QGstreamerVideoWidgetControl::onNativeVideoSizeChanged()
{
    const iSize &size = m_videoOverlay.nativeVideoSize();
...
bool QGstreamerVideoWidgetControl::eventFilter(iObject *object, QEvent *e)
{
...
bool QGstreamerVideoWidgetControl::processSyncMessage(const iGstreamerMessage &message)
{
    return m_videoOverlay.processSyncMessage(message);
}

bool QGstreamerVideoWidgetControl::processBusMessage(const iGstreamerMessage &message)
{

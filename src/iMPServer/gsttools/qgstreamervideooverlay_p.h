...
class QGstreamerVideoOverlay
        ...
        , private iGstreamerBufferProbe
{
    Q_OBJECT
    Q_INTERFACES(iGstreamerSyncMessageFilter iGstreamerBusMessageFilter)
public:
    explicit QGstreamerVideoOverlay(iObject *parent = 0, const iByteArray &elementName = iByteArray());
    virtual ~QGstreamerVideoOverlay();

    GstElement *videoSink() const;
    void setVideoSink(GstElement *);
    iSize nativeVideoSize() const;

    void setWindowHandle(WId id);
    void expose();
    void setRenderRectangle(const iRect &rect);

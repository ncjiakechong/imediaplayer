...
static QGstreamerMirTextureRenderer *rendererInstance = IX_NULLPTR;

class QGstreamerMirTextureBuffer : public iAbstractVideoBuffer
{
public:
    QGstreamerMirTextureBuffer(GLuint textureId) :
        iAbstractVideoBuffer(iAbstractVideoBuffer::GLTextureHandle),
    ...
    uchar *map(MapMode mode, int *numBytes, int *bytesPerLine)
    {
        qDebug() << Q_FUNC_INFO;
        IX_GCC_UNUSED(mode);
        IX_GCC_UNUSED(numBytes);
        IX_GCC_UNUSED(bytesPerLine);
    ...
    iVariant handle() const { return iVariant::fromValue<unsigned int>(m_textureId); }
...
GstElement *QGstreamerMirTextureRenderer::videoSink()
{
    ...
    if (m_videoSink) {
        ...
        gst_pad_add_probe(pad, GST_PAD_PROBE_TYPE_BUFFER,
                padBufferProbe, this, IX_NULLPTR);
    }
...
#if 1
    iScopedLock locker(&rendererInstance->m_mutex);
    QMetaObject::invokeMethod(rendererInstance, "renderFrame", Qt::QueuedConnection);
#else
    // FIXME!
    //iScopedLock locker(&renderer->m_mutex);
...
void QGstreamerMirTextureRenderer::renderFrame()
{
    ...
    if (!m_surface->isActive()) {
        ...
        if (caps) {
            ...
            iSize newNativeSize = iGstUtils::capsCorrectedResolution(caps);
        ...
        // Start the surface
        iVideoSurfaceFormat format(m_nativeSize, iVideoFrame::Format_RGB32, iAbstractVideoBuffer::GLTextureHandle);
    ...
    iVideoFrame frame(buffer, m_surface->surfaceFormat().frameSize(),
...
{
    //qDebug() << Q_FUNC_INFO;
    const iSize oldSize = m_nativeSize;
    ...
    if (m_videoSink) {
        ...
        if (caps) {
            m_nativeSize = iGstUtils::capsCorrectedResolution(caps);
            gst_caps_unref(caps);
        }
    } else {
        m_nativeSize = iSize();
...
void QGstreamerMirTextureRenderer::handleFocusWindowChanged(QWindow *window)
{
    ...
    if (!currContext && !m_glContext) {
        ...
        if (m_surface)
            shareContext = qobject_cast<QOpenGLContext*>(m_surface->property("GLContext").value<iObject*>());
    ...
    if (m_textureId == 0) {
        ...
        g_object_set(G_OBJECT(m_videoSink), "texture-id", m_textureId, (char*)IX_NULLPTR);
    }
}

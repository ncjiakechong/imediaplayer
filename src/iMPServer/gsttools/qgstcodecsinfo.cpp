...
std::list<iString> iGstCodecsInfo::codecOptions(const iString &codec) const
{
    ...
    iByteArray elementName = m_codecInfo.value(codec).elementName;
    if (elementName.isEmpty())
        return options;

    GstElement *element = gst_element_factory_make(elementName, IX_NULLPTR);
    if (element) {
        guint numProperties;
        ...
        for (guint j = 0; j < numProperties; ++j) {
            ...
            options.append(iLatin1String(property->name));
        }
        g_free(properties);
        ...
        while (padTemplates) {
            ...
            if (padTemplate->direction == GST_PAD_SRC) {
                ...
                for (uint i=0; i<gst_caps_get_size(caps); i++) {
                    ...
                    std::multimap<iString, CodecInfo>::const_iterator it = m_codecInfo.constFind(codec);

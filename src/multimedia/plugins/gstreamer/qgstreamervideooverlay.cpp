...
public:
    ...
    int brightness() const override
    {
        int brightness = 0;
        if (m_hasBrightness)
            g_object_get(G_OBJECT(m_videoSink), "brightness", &brightness, IX_NULLPTR);
    ...
    bool setBrightness(int brightness) override
    {
        ...
        if (m_hasBrightness)
            g_object_set(G_OBJECT(m_videoSink), "brightness", brightness * 10, IX_NULLPTR);
    ...
    int contrast() const override
    {
        int contrast = 0;
        if (m_hasContrast)
            g_object_get(G_OBJECT(m_videoSink), "contrast", &contrast, IX_NULLPTR);
    ...
    bool setContrast(int contrast) override
    {
        m_contrast = contrast;
        if (m_hasContrast)
            g_object_set(G_OBJECT(m_videoSink), "contrast", contrast * 10, IX_NULLPTR);
    ...
    int hue() const override
    {
        int hue = 0;
        if (m_hasHue)
            g_object_get(G_OBJECT(m_videoSink), "hue", &hue, IX_NULLPTR);
    ...
    bool setHue(int hue) override
    {
        m_hue = hue;
        if (m_hasHue)
            g_object_set(G_OBJECT(m_videoSink), "hue", hue * 10, IX_NULLPTR);
    ...
    int saturation() const override
    {
        int saturation = 0;
        if (m_hasSaturation)
            g_object_get(G_OBJECT(m_videoSink), "saturation", &saturation, IX_NULLPTR);
    ...
    bool setSaturation(int saturation) override
    {
        ...
        if (m_hasSaturation)
            g_object_set(G_OBJECT(m_videoSink), "saturation", saturation * 10, IX_NULLPTR);
    ...
    Qt::AspectRatioMode aspectRatioMode() const override
    {
        ...
        if (m_hasForceAspectRatio) {
            ...
            g_object_get(G_OBJECT(m_videoSink), "force-aspect-ratio", &forceAR, IX_NULLPTR);
            if (!forceAR)
...
public:
    ...
    bool setBrightness(int brightness) override
    {
        ...
        if (m_hasBrightness) {
            ...
            g_object_set(G_OBJECT(m_videoSink), "brightness", v, IX_NULLPTR);
        }
    ...
    bool setContrast(int contrast) override
    {
        ...
        if (m_hasContrast) {
            ...
            g_object_set(G_OBJECT(m_videoSink), "contrast", v, IX_NULLPTR);
        }
    ...
    bool setHue(int hue) override
    {
        ...
        if (m_hasHue) {
            ...
            g_object_set(G_OBJECT(m_videoSink), "hue", v, IX_NULLPTR);
        }
    ...
    bool setSaturation(int saturation) override
    {
        ...
        if (m_hasSaturation) {
            ...
            g_object_set(G_OBJECT(m_videoSink), "saturation", v, IX_NULLPTR);
        }
...
{
    GstElement *choice = 0;
    iString platform = QGuiApplication::platformName();
    ...
    for (quint32 i = 0; i < (sizeof(elementMap) / sizeof(ElementMap)); ++i) {
        if (platform == iLatin1String(elementMap[i].qtPlatform)
                && (choice = gst_element_factory_make(elementMap[i].gstreamerElement, IX_NULLPTR))) {
    ...
    GList *list = ix_gst_video_sinks();
    for (GList *item = list; item != IX_NULLPTR; item = item->next) {
        GstElementFactory *f = GST_ELEMENT_FACTORY(item->data);

        if (!gst_element_factory_has_interface(f, IX_GSTREAMER_VIDEOOVERLAY_INTERFACE_NAME))
            continue;

        if (GstElement *el = gst_element_factory_create(f, IX_NULLPTR)) {
...
QGstreamerVideoOverlay::QGstreamerVideoOverlay(iObject *parent, const iByteArray &elementName)
    : iObject(parent)
    , iGstreamerBufferProbe(iGstreamerBufferProbe::ProbeCaps)
{
    GstElement *sink = nullptr;
    if (!elementName.isEmpty())
        sink = gst_element_factory_make(elementName.constData(), IX_NULLPTR);
    else
...
void QGstreamerVideoOverlay::setVideoSink(GstElement *sink)
{
    ...
    m_videoSink = sink;
    ix_gst_object_ref_sink(GST_OBJECT(m_videoSink));

    GstPad *pad = gst_element_get_static_pad(m_videoSink, "sink");
    addProbeToPad(pad);
    gst_object_unref(GST_OBJECT(pad));

    iString sinkName(iLatin1String(GST_OBJECT_NAME(sink)));
    bool isVaapi = sinkName.startsWith(iLatin1String("vaapisink"));
    delete m_sinkProperties;
...
iSize QGstreamerVideoOverlay::nativeVideoSize() const
{
...
void QGstreamerVideoOverlay::setRenderRectangle(const iRect &rect)
{
    int x = -1;
    int y = -1;
    int w = -1;
...
bool QGstreamerVideoOverlay::processSyncMessage(const iGstreamerMessage &message)
{
...
bool QGstreamerVideoOverlay::processBusMessage(const iGstreamerMessage &message)
{
    GstMessage* gm = message.rawMessage();
...
{
    iSize size = iGstUtils::capsCorrectedResolution(caps);
...
void QGstreamerVideoOverlay::updateIsActive()
{
    ...
    if (m_sinkProperties->hasShowPrerollFrame())
        g_object_get(G_OBJECT(m_videoSink), "show-preroll-frame", &showPreroll, IX_NULLPTR);

    ...
    , m_stream(0)
{
    ...
    IX_ASSERT(m_resources);

    connect(m_session, SIGNAL(positionChanged(xint64)),
            this, SIGNAL(positionChanged(xint64)));
    connect(m_session, SIGNAL(durationChanged(xint64)),
            this, SIGNAL(durationChanged(xint64)));
    ...
    connect(m_session, SIGNAL(error(int,iString)),
            this, SIGNAL(error(int,iString)));
    connect(m_session, SIGNAL(invalidMedia()),
            this, SLOT(handleInvalidMedia()));
    connect(m_session, SIGNAL(playbackRateChanged(xreal)),
            this, SIGNAL(playbackRateChanged(xreal)));
...
xint64 QGstreamerPlayerControl::position() const
{
...
xint64 QGstreamerPlayerControl::duration() const
{
    return m_session->duration();
}
...
xreal QGstreamerPlayerControl::playbackRate() const
{
    return m_session->playbackRate();
}

void QGstreamerPlayerControl::setPlaybackRate(xreal rate)
{
    m_session->setPlaybackRate(rate);
}

void QGstreamerPlayerControl::setPosition(xint64 pos)
{
#ifdef DEBUG_PLAYBIN
...
void QGstreamerPlayerControl::setVideoOutput(iObject *output)
{
...
{
    IX_ASSERT(!m_stateStack.isEmpty());

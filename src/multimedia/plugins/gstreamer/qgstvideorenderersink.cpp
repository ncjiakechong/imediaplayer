...
{
    return iGstUtils::capsForFormats(surface->supportedPixelFormats());
}

bool QGstDefaultVideoRenderer::start(QAbstractVideoSurface *surface, GstCaps *caps)
{
    m_flushed = true;
    m_format = iGstUtils::formatForCaps(caps, &m_videoInfo);
...
Q_GLOBAL_STATIC_WITH_ARGS(QMediaPluginLoader, rendererLoader,
        (QGstVideoRendererInterface_iid, iLatin1String("video/gstvideorenderer"), Qt::CaseInsensitive))
...
bool QVideoSurfaceGstDelegate::event(QEvent *event)
{
    if (event->type() == QEvent::UpdateRequest) {
        ...
        return iObject::event(event);
    }
}

bool QVideoSurfaceGstDelegate::handleEvent(iScopedLock *locker)
{
    if (m_flush) {
        m_flush = false;
        if (m_activeRenderer) {
...
bool QVideoSurfaceGstDelegate::waitForAsyncEvent(
        iScopedLock *locker, QWaitCondition *condition, unsigned long time)
{

...
static void mapToLowerCase(iString *str, int from)
{
    ...
    for (int i = from; i < str->size(); ++i) {
        ...
        if (uc < 0x80) {
            ...
            if ((entry != NameprepCaseFolding + N) && !(uc < *entry)) {
                ...
                if (l > 1 || uc > 0xffff) {
                    if (uc <= 0xffff)
                        str->replace(i, 1, reinterpret_cast<const iChar *>(&entry->mapping[0]), l);
                    else
                        str->replace(--i, 2, reinterpret_cast<const iChar *>(&entry->mapping[0]), l);
                    i += l - 1;
                    d = 0;
                } else {
                    if (!d)
...
{
    iChar *src = source->data(); // causes a detach, so we're sure the only one using it
    iChar *out = src + from;
    ...
    for ( ; in < e; ++in) {
        ...
        if (!isUnassignedAllowed) {
            iChar::UnicodeVersion version = iChar::unicodeVersion(uc);
            if (version == iChar::Unicode_Unassigned || version > iChar::Unicode_3_2) {
...
static bool ix_check_nameprepped_std3(const iChar *in, int len)
{
    ...
    const iChar *e = in + len;
    iString origin = iString::fromRawData(firstNonAscii, e - firstNonAscii);
    iString copy = origin;
    ix_nameprep(&copy, 0);
    return origin == copy;
}
...
iString ix_punycodeDecoder(const iString &pc)
{
    ...
    int delimiterPos = pc.lastIndexOf(iChar(0x2d));
    iString output = delimiterPos < 4 ?
                     iString() : pc.mid(start, delimiterPos - start);
...
static bool ix_is_idn_enabled(const iString &domain)
{
    ...
    iString tldString = ix_ACE_do(iString::fromRawData(domain.constData() + idx + 1, len), ToAceOnly, ForbidLeadingDot);
    len = tldString.size();
...
static int nextDotDelimiter(const iString &domain, int from = 0)
{
    const iChar *b = domain.unicode();
    const iChar *ch = b + from;
    const iChar *e = b + domain.length();
    while (ch < e) {
...
iString ix_ACE_do(const iString &domain, AceOperation op, AceLeadingDot dot)
{
    ...
    while (1) {
        ...
        if (simple) {
            ...
            // so we're done
            if (!ix_check_std3rules(result.constData() + prevLen, labelLength))
                return iString();
        } else {
            // Punycode encoding and decoding cannot be done in-place
            // That means we need one or two temporaries
            if (!ix_nameprep(&result, prevLen))
                return iString();   // failed
            ...
            ix_punycodeEncoder(result.constData() + prevLen, result.size() - prevLen, &aceForm);
            ...
            if (!appended) {
                ...
                memcpy(result.data() + prevLen, aceForm.constData(), aceForm.size() * sizeof(iChar));
            }

            if (!ix_check_std3rules(aceForm.constData(), aceForm.size()))
                return iString();
        }

...
void iIODevice::setCurrentReadChannel(int channel)
{
    if (d_ptr->transactionStarted) {
        ilog_warn(this, "setReadChannel", "Failed due to read transaction being in progress");
        return;
    }
...
bool iIODevice::seek(xint64 pos)
{
    if (d_ptr->isSequential()) {
        ilog_warn("seek", "Cannot call seek on a sequential device");
        return false;
    }
    if (d_ptr->openMode == NotOpen) {
        ilog_warn("seek", "The device is not open");
        return false;
    }
    if (pos < 0) {
        ilog_warn("iIODevice::seek: Invalid pos: %lld", pos);
        return false;
    }
...
{
    if (maxSize < 2) {
        ilog_warn(this, "readLine", "Called with maxSize < 2");
        return xint64(-1);
    }
...
{
    if (d_ptr->transactionStarted) {
        ilog_warn("startTransaction", "Called while transaction already in progress");
        return;
    }
...
{
    if (!d_ptr->transactionStarted) {
        ilog_warn("commitTransaction", "Called while no transaction in progress");
        return;
    }
    if (d_ptr->isSequential())
...
{
    if (!d_ptr->transactionStarted) {
        ilog_warn("rollbackTransaction", "Called while no transaction in progress");
        return;
    }
    if (!d_ptr->isSequential())

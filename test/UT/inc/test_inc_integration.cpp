...
protected:
                     ...
                     xuint16 version, const iByteArray& args) override
    {
        ...
        conn->sendReply(seqNum, INC_OK, args);
    }
...
TEST_P(INCIntegrationTest, StreamOperationsWhenDetached) {
    ...
    iByteArray readData = stream->read();
    EXPECT_TRUE(readData.isEmpty());
    
    iByteArray peekData = stream->peek();
    EXPECT_TRUE(peekData.isEmpty());
